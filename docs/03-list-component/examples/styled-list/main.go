...
var (
	...
	// Age-based emoji styles
	youngStyle  = lipgloss.NewStyle().Foreground(lipgloss.Color("#FFD700")) // Gold
	midStyle    = lipgloss.NewStyle().Foreground(lipgloss.Color("#FFA500")) // Orange
	seniorStyle = lipgloss.NewStyle().Foreground(lipgloss.Color("#9370DB")) // Purple
)
...
// Get age-specific style with emoji and color
func getAgeStyle(age int) (lipgloss.Style, string) {
	if age < 30 {
		return youngStyle, fmt.Sprintf("(%d) 🌟", age) // Young professional
	} else if age > 45 {
		return seniorStyle, fmt.Sprintf("(%d) 👑", age) // Senior professional
	} else {
		return midStyle, fmt.Sprintf("(%d)", age) // Mid-career
	}
}

// Beautiful styled formatter function - simplified approach
func styledPersonFormatter(data core.Data[any], index int, ctx core.RenderContext, isCursor, isTopThreshold, isBottomThreshold bool) string {
	...
	// Use simple foreground colors only - no backgrounds to avoid width issues
	var nameColor, ageColor, jobColor, cityColor lipgloss.Color

	if isCursor {
		// Bright colors for cursor
		nameColor = "#FFFF00" // Bright Yellow
		ageColor = "#FFD700"  // Gold
		jobColor = "#00FFFF"  // Cyan
		cityColor = "#00FF00" // Bright Green
	} else if data.Selected {
		// Different bright colors for selected
		nameColor = "#FF69B4" // Hot Pink
		ageColor = "#FFA500"  // Orange
		jobColor = "#87CEEB"  // Sky Blue
		cityColor = "#98FB98" // Pale Green
	} else {
		// Normal colors
		nameColor = "#FFFFFF" // White
		ageColor = getAgeColor(person.Age)
		jobColor = getJobColor(person.Job)
		cityColor = "#98FB98" // Light Green
	}

	// Apply simple foreground styling
	styledName := lipgloss.NewStyle().Foreground(nameColor).Bold(true).Render(person.Name)

	// Age with emoji
	ageText := getAgeText(person.Age)
	styledAge := lipgloss.NewStyle().Foreground(ageColor).Render(ageText)
...
func (app *App) View() string {
	...
	title := titleStyle.Render("🎨 Styled List with Beautiful Colors")
