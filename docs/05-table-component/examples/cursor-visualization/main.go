...
type AppModel struct {
	...
	// ONLY cursor visualization fields
...
func main() {
	...
	columns := []core.TableColumn{
		{Title: "Employee Name", Width: 20, Alignment: core.AlignLeft},
		{Title: "Department", Width: 15, Alignment: core.AlignCenter},
		{Title: "Status", Width: 12, Alignment: core.AlignCenter},
		{Title: "Salary", Width: 12, Alignment: core.AlignRight},
		{Title: "Description", Width: 35, Alignment: core.AlignLeft},
	}

	activeCellColors := []string{"#3C3C3C", "#1E3A8A", "#166534", "#7C2D12", "#581C87"}

	// Simple default theme (since this builds on table-styling example)
	theme := core.Theme{
	...
	config := core.TableConfig{
		...
		ShowHeader:                  true,
		ShowBorders:                 true,
		FullRowHighlighting:         true,  // Start with full row highlighting enabled
		ActiveCellIndicationEnabled: false, // Start with active cell disabled
	...
	model := AppModel{
		...
		statusMessage:           "Cursor Visualization Demo - Press r (row highlight), c (cell indication), C (colors), m (mixed mode)",
	...
	case tea.KeyMsg:
		...
		case "r":
			...
			if m.fullRowHighlightEnabled {
				...
				m.statusMessage = "Full row highlighting: ENABLED - entire row highlighted"
		...
		case "c":
			...
			if m.activeCellEnabled {
				...
				m.statusMessage = "Active cell indication: ENABLED (background highlighting)"
		...
		case "m":
			...
			if m.fullRowHighlightEnabled && m.activeCellEnabled {
				...
				m.fullRowHighlightEnabled = false
				m.activeCellEnabled = false
				m.statusMessage = "Mixed mode: DISABLED (both off)"
				return m, tea.Batch(
					core.FullRowHighlightEnableCmd(false),
					core.ActiveCellIndicationModeSetCmd(false),
				)
			} else if m.fullRowHighlightEnabled || m.activeCellEnabled {
				// One on, turn both on
				m.fullRowHighlightEnabled = true
				m.activeCellEnabled = true
				m.statusMessage = "Mixed mode: ENABLED (both row highlight and active cell)"
				return m, tea.Batch(
...
func (m AppModel) View() string {
	// Status line showing current cursor visualization mode
	var mode string
	if m.fullRowHighlightEnabled && m.activeCellEnabled {
		mode = "mixed"
	...
	status := fmt.Sprintf("Employee %d/%d | Mode: %s | Color: %s",
		state.CursorIndex+1,
	...
	controls := "r=toggle row highlighting | c=toggle cell indication | C=cycle colors | m=mixed mode | ↑↓jk=move | hl=page | gG=start/end | q=quit"
