...
func NewTestDataSource(items []core.TableRow) *TestDataSource {
...
func (ds *TestDataSource) GetTotal() tea.Cmd {
	return func() tea.Msg {
		return core.DataTotalMsg{Total: ds.totalItems}
	}
}

func (ds *TestDataSource) LoadChunk(request core.DataRequest) tea.Cmd {
	return func() tea.Msg {
...
func createTestTable(rows []core.TableRow) *Table {
	columns := []core.TableColumn{
		{Title: "Name", Field: "name", Width: 10, Alignment: core.AlignLeft},
		{Title: "Value", Field: "value", Width: 8, Alignment: core.AlignRight},
		{Title: "Status", Field: "status", Width: 10, Alignment: core.AlignCenter},
	}
	...
	// Load first chunk
	chunkMsg := dataSource.LoadChunk(core.DataRequest{
		Start: 0,
		Count: 10,
	})()
...
func createTestRows(count int) []core.TableRow {
	rows := make([]core.TableRow, count)
	for i := 0; i < count; i++ {
		rows[i] = core.TableRow{
...
func TestTable_CellFormatters(t *testing.T) {
	...
	formatterMsg := core.CellFormatterSetCmd(0, nameFormatter)()
	...
	// Apply formatter through the Tea model system
	formatterMsg2 := core.CellFormatterSetCmd(1, valueFormatter)()
	...
	if !strings.Contains(output, "• Item 1") {
		t.Errorf("Expected formatted name content with bullet point, got: %s", output)
	}
	if !strings.Contains(output, "$10") {
		t.Errorf("Expected formatted value content with dollar sign, got: %s", output)
	}
}
...
func TestTable_Selection(t *testing.T) {
	...
	selectMsg := core.SelectCurrentMsg{}
	...
	selectionMsg := core.SelectionResponseMsg{Success: true, Index: 1, Selected: true}
	table.Update(selectionMsg)

	// Reload chunk to get updated selection state
	chunkMsg := table.dataSource.LoadChunk(core.DataRequest{
		Start: 0,
		Count: 10,
	})()
	table.Update(chunkMsg)

	// Test that selection count increased
	if table.GetSelectionCount() != 1 {
		t.Errorf("Expected 1 selected item, got %d", table.GetSelectionCount())
	}
}
...
func TestTable_LoadingState(t *testing.T) {
	...
	table := createTestTable([]core.TableRow{})

	// Set total but don't load chunks yet
	totalMsg := core.DataTotalMsg{Total: 5}
	table.Update(totalMsg)
...
func TestTable_HeaderFormatters(t *testing.T) {
	...
	headerFormatter := func(column core.TableColumn, ctx core.RenderContext) string {
		return "📋 " + column.Title
	}
	...
	if !strings.Contains(output, "📋 Name") {
		t.Errorf("Expected formatted header with clipboard icon, got: %s", output)
	}
}
...
func TestTable_FullIntegration(t *testing.T) {
	...
	// Select an item
	table.dataSource.SetSelected(1, true)
	table.Update(core.SelectionResponseMsg{Success: true, Index: 1, Selected: true})
