...
func (s *EnhancedDataSource) LoadChunk(request core.DataRequest) tea.Cmd {
	return func() tea.Msg {
...
func (s *EnhancedDataSource) LoadChunkImmediate(request core.DataRequest) core.DataChunkLoadedMsg {
	start := request.Start
	count := request.Count
	total := s.totalTasks
...
func (s *EnhancedDataSource) GetTotal() tea.Cmd {
	return core.DataTotalCmd(s.totalTasks)
}
...
func (s *EnhancedDataSource) SetSelected(index int, selected bool) tea.Cmd {
	...
	return core.SelectionResponseCmd(true, index, fmt.Sprintf("task-%d", index), selected, "toggle", nil, nil)
}
...
type ComponentDemoModel struct {
	...
	renderConfigs      []core.ListRenderConfig
	...
	componentOrders   [][]core.ListComponentType
...
func main() {
	...
	renderConfigs := []core.ListRenderConfig{
		...
		list.BulletListConfig(),

		// 2. Numbered list with alignment
		list.NumberedListConfig(),

		// 3. Checklist style
		list.ChecklistConfig(),

		// 4. Minimal (content only)
		list.MinimalListConfig(),

		// 5. Custom enumerator with arrows
		func() core.ListRenderConfig {
			config := config.DefaultListRenderConfig()
			config.EnumeratorConfig.Enumerator = func(data core.Data[any], index int, ctx core.RenderContext) string {
				return "→ "
			}
			return config
		}(),
		...
		func() core.ListRenderConfig {
			config := config.DefaultListRenderConfig()
			conditionalEnum := list.NewConditionalEnumerator(list.BulletEnumerator).
				When(list.IsSelected, list.CheckboxEnumerator).
				When(list.IsError, func(item core.Data[any], index int, ctx core.RenderContext) string {
					return "❌ "
				}).
				When(list.IsLoading, func(item core.Data[any], index int, ctx core.RenderContext) string {
					return "⏳ "
				})
	...
	componentOrders := [][]core.ListComponentType{
		// Standard order
		{core.ListComponentCursor, core.ListComponentEnumerator, core.ListComponentContent},
		// Content first
		{core.ListComponentContent, core.ListComponentEnumerator, core.ListComponentCursor},
		// Enumerator first
		{core.ListComponentEnumerator, core.ListComponentCursor, core.ListComponentContent},
		// Content only
		{core.ListComponentContent},
		// Cursor and content only
		{core.ListComponentCursor, core.ListComponentContent},
		// With spacing
		{core.ListComponentCursor, core.ListComponentPreSpacing, core.ListComponentEnumerator, core.ListComponentContent, core.ListComponentPostSpacing},
	}
...
func (m ComponentDemoModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "1":
			return m, core.JumpToCmd(20)
		case "2":
			return m, core.JumpToCmd(50)
		case "3":
			return m, core.JumpToCmd(80)
		case "4":
			return m, core.JumpToCmd(120)
		case "5":
			return m, core.JumpToCmd(140)
	...
	case BackgroundModeChangeMsg:
		...
		case 1: // Entire Line
			...
			config.BackgroundConfig.Mode = core.ListBackgroundEntireLine
			config.BackgroundConfig.Style = lipgloss.NewStyle().
				Background(lipgloss.Color("240")).
				Foreground(lipgloss.Color("15"))
		case 2: // Content Only
			config.BackgroundConfig.Enabled = true
			config.BackgroundConfig.Mode = core.ListBackgroundContentOnly
			config.BackgroundConfig.Style = lipgloss.NewStyle().
				Background(lipgloss.Color("33")).
				Foreground(lipgloss.Color("15"))
		case 3: // Indicator Only
			config.BackgroundConfig.Enabled = true
			config.BackgroundConfig.Mode = core.ListBackgroundIndicatorOnly
			config.BackgroundConfig.Style = lipgloss.NewStyle().
	...
	case EnumeratorAlignmentToggleMsg:
		config := m.list.GetRenderConfig()
		if config.EnumeratorConfig.Alignment == core.ListAlignmentNone {
			config.EnumeratorConfig.Alignment = core.ListAlignmentRight
			...
			config.EnumeratorConfig.Alignment = core.ListAlignmentNone
...
func createTaskFormatter() core.ItemFormatter[any] {
	return func(
		item core.Data[any],
		index int,
