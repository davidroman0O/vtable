...
func initialModel() model {
	...
	// Create list config with better settings for tables
	listConfig := vtable.NewListConfigBuilder().
		WithViewportHeight(15).
		WithChunkSize(25). // Smaller chunks for smoother scrolling
		WithSelectionMode(vtable.SelectionMultiple).
		WithMaxWidth(200). // Wider for tables
		Build()

	// Create table config
	tableConfig := vtable.DefaultTableConfig()

	// Create table with enhanced rendering
	table := vtable.NewTable(listConfig, tableConfig, dataSource)

	// CRITICAL: Enable the enhanced rendering system with alternating rows
