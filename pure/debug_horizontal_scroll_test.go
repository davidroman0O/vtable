...
func (ds *DebugDataSource) LoadChunk(request DataRequest) tea.Cmd {
	return func() tea.Msg {
		start := request.Start
...
func TestDebugHorizontalScrolling_StepByStep(t *testing.T) {
	...
	// Load data
	chunkCmd := dataSource.LoadChunk(DataRequest{Start: 0, Count: 1})
	if chunkCmd != nil {
		msg := chunkCmd()
		if msg != nil {
			table.Update(msg)
...
func TestDebugHorizontalScrolling_ExactDemoSetup(t *testing.T) {
	...
	columns := []TableColumn{
		{
			...
			HeaderConstraint: CellConstraint{
				...
				Padding:   PaddingConfig{Left: 1, Right: 1},
			},
		},
		{
			Title:           "Value",
		...
		},
		{
			...
			HeaderConstraint: CellConstraint{
				Width:     20,
				Alignment: AlignCenter,
				Padding:   PaddingConfig{Left: 2, Right: 2},
			},
		},
		{
			Title:           "Description",
	...
	table.Update(CellFormatterSetCmd(4, descriptionFormatter)())
	...
	table.Update(HeaderFormatterSetCmd(3, func(column TableColumn, ctx RenderContext) string {
		return "🏷️ " + column.Title
	})())
	table.Update(HeaderFormatterSetCmd(4, func(column TableColumn, ctx RenderContext) string {
		return "📄 " + column.Title
	})())
...
func TestDebugHorizontalScrolling_IsolateStylingBug(t *testing.T) {
	...
	columns := []TableColumn{
		...
		{Title: "Description", Field: "description", Width: 22, Alignment: AlignLeft},
	}
	...
	config := TableConfig{
		...
		ShowHeader:     true,
		ShowBorders:    true,
		ViewportConfig: ViewportConfig{Height: 15, ChunkSize: 100},
		Theme:          DefaultTheme(),
		SelectionMode:  SelectionMultiple,
	}
	...
	chunkCmd := dataSource1.LoadChunk(DataRequest{Start: 0, Count: 1})
	if chunkCmd != nil {
		msg := chunkCmd()
		if msg != nil {
			table1.Update(msg)
	...
	table2.Update(CellFormatterSetCmd(4, descriptionFormatter)())
	...
	chunkCmd2 := dataSource2.LoadChunk(DataRequest{Start: 0, Count: 1})
	if chunkCmd2 != nil {
		msg := chunkCmd2()
		if msg != nil {
			table2.Update(msg)
...
func TestDebugHorizontalScrolling_ColumnWidthBug(t *testing.T) {
	...
	columns1 := []TableColumn{
		{Title: "Description", Field: "description", Width: 25, Alignment: AlignLeft},
	}
	...
	config1 := TableConfig{
		...
		ShowHeader:     true,
		ShowBorders:    true,
		ViewportConfig: ViewportConfig{Height: 3, ChunkSize: 10},
		Theme:          DefaultTheme(),
		SelectionMode:  SelectionNone,
	}
	...
	columns2 := []TableColumn{
		{Title: "Description", Field: "description", Width: 22, Alignment: AlignLeft}, // Same width as demo
	}
...
func TestDebugUserIssue_ItemFiveStuck(t *testing.T) {
	...
	var demoData []TableRow
	for i := 0; i < 10; i++ {
		demoData = append(demoData, TableRow{
	...
	chunkCmd := dataSource.LoadChunk(DataRequest{Start: 0, Count: 10})
	if chunkCmd != nil {
		msg := chunkCmd()
		if msg != nil {
			table.Update(msg)
...
func TestRowSpecificHorizontalScrolling(t *testing.T) {
	...
	var testData []TableRow
	for i := 0; i < 5; i++ {
		testData = append(testData, TableRow{
			ID: fmt.Sprintf("row-%d", i),
			Cells: []string{
	...
	columns := []TableColumn{
		{Title: "Name", Field: "name", Width: 15, Alignment: AlignLeft}, // Narrow to force scrolling
		{Title: "Value", Field: "value", Width: 10, Alignment: AlignRight},
		{Title: "Status", Field: "status", Width: 12, Alignment: AlignCenter},
		{Title: "Category", Field: "category", Width: 10, Alignment: AlignLeft},
		{Title: "Description", Field: "description", Width: 20, Alignment: AlignLeft},
	}
	...
	chunkCmd := dataSource.LoadChunk(DataRequest{Start: 0, Count: 5})
	if chunkCmd != nil {
		msg := chunkCmd()
		if msg != nil {
			table.Update(msg)
...
func TestDemoScrollingProblemDiagnosis(t *testing.T) {
	...
	if len(descContent) > descColumnWidth {
		...
		config := TableConfig{
			...
			ShowHeader:     true,
			ShowBorders:    true,
			ViewportConfig: ViewportConfig{Height: 5, ChunkSize: 10},
			Theme:          DefaultTheme(),
			SelectionMode:  SelectionNone,
		}
...
func TestDebugCursorRowScrolling(t *testing.T) {
	...
	columns := []TableColumn{
		{Title: "Name", Field: "name", Width: 10, Alignment: AlignLeft}, // Narrow to force scrolling
	}
	...
	config := TableConfig{
		...
		ShowHeader:     true,
		ShowBorders:    true,
		ViewportConfig: ViewportConfig{Height: 8, ChunkSize: 10},
		Theme:          DefaultTheme(),
		SelectionMode:  SelectionNone,
	}
...
func TestHorizontalScrollingModesVerification(t *testing.T) {
	...
	columns := []TableColumn{
		{Title: "Name", Field: "name", Width: 8, Alignment: AlignLeft}, // Narrow to force scrolling
	}
